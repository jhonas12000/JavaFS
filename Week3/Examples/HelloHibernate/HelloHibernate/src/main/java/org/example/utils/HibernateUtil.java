...
public class HibernateUtil {
    ...
    //Note that we point it to the ibernate.cfg.xml file where our database credentials etc. are stored

    private static SessionFactory sf = new Configuration().configure("hibernate.cfg.xml").buildSessionFactory();


    //get Session objects from our SessionFactory
    //We only want one session in this applicatoin - hence private static - it's a singleton!
    ...
    public static void closeSession(){
        if (ses!=null){
            ses.close();
            ses=null; //reset it to null so we can get a new one later if needed
        }
    }
}
